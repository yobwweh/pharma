...
const dbPath = require$$1$1.join(app.getPath("userData"), "pharma.db");
const dbUrl = `file:${isDev ? require$$1$1.join(__dirname$1, "../../prisma/dev.db") : dbPath}`;
...
const absoluteDbPath = isDev ? require$$1$1.resolve(__dirname$1, "../prisma/dev.db") : require$$1$1.resolve(app.getPath("userData"), "pharma.db");
const dbDir = require$$1$1.dirname(absoluteDbPath);
if (!existsSync(dbDir)) {
  console.log(`Création du dossier de base de données : ${dbDir}`);
  const { mkdirSync } = requireNode("fs");
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    height: 800,
    webPreferences: {
      preload: require$$1$1.join(__dirname$1, "preload.js"),
...
ipcMain.handle("check-updates", async () => {
  try {
    ...
    console.error("Erreur lors de la vérification des mises à jour :", error2);
    throw error2;
  }
});
ipcMain.handle("get-open-shift", async (_evt, userId) => {
  try {
    const shift = await prisma.cashShift.findFirst({ where: { userId, status: "OPEN" } });
    return shift;
  } catch (error2) {
    console.error("get-open-shift error", error2);
    throw error2;
  }
});
ipcMain.handle("open-shift", async (_evt, { userId, openingBalance }) => {
  try {
    ...
    console.error("open-shift error", error2);
    throw error2;
  }
});
ipcMain.handle("close-shift", async (_evt, { shiftId, actualCashCount }) => {
...
ipcMain.handle("process-sale", async (_evt, { cartItems, userId, shiftId, paymentMethod, total }) => {
  return prisma.$transaction(async (tx) => {
    const shift = await tx.cashShift.findUnique({ where: { id: shiftId } });
